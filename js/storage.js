...
export const storage = {
    getDefaultSettings() {
        return {
            ...
            kelompoks: [
                "Tim Yudi", "Tim Aziz", "Tim Sukma", "Tim Rfaid", "Tim Arya",
                "Tim Dafa", "Tim Farel", "Tim P.Deni", "Proyek A", "Operasional Kantor",
            ],
        };
    },
    ...
    findLatestSettings() {
        const keys = Object.keys(localStorage).filter(
            (k) => k.startsWith("accountingApp_") && k !== `accountingApp_${state.currentMonth}`
        );
    ...
    findPreviousPeriodData(targetMonth) {
        const keys = Object.keys(localStorage).filter(
            (k) => k.startsWith("accountingApp_") && k < `accountingApp_${targetMonth}`
        );
    ...
    calculateStockLevels(items, transactions) {
        ...
        transactions.forEach((trx) => {
            if (stockData[trx.kode]) {
                ...
                else if (trx.tipe === "keluar") stockData[trx.kode].keluar += trx.jumlah;
            }
        });
    ...
    saveDataForCurrentMonth() {
        ...
        utils.showModal(
            "Sukses",
            `Data untuk periode ${utils.formatMonth(state.currentMonth)} telah berhasil disimpan.`
        );
    },
    ...
    exportData() {
        ...
        link.href = url;
        link.download = `data_akuntansi_backup_${new Date().toISOString().slice(0, 10)}.json`;
